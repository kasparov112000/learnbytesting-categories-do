...
type GridServerSideRowRequest = Partial<IServerSideGetRowsRequest> & { search: { search: string; inflightStart: Date; inflightEnd: Date; }; };
...
export class CategoryService extends DbMicroServiceBase { // eslint-disable-line
  ...
  public async grid(params: GridServerSideRowRequest, res, txnId): Promise<any> {
  ...
  private getGridFilter(params: GridServerSideRowRequest, txnId, isFlatQuery: boolean): Record<string, string> {
    ...
    if (!isFlatQuery && params && params.search?.search) {
      query.$text = { $search: `"${trim(params.search?.search)?.replace('"', '\\"')}"` };
    }
    ...
    for (const entry of entries) {
      const key = entry[0];
      const value = entry[1];
      const operator = value.operator === 'AND' ? '$and' : value.operator === 'OR' ? '$or' : undefined;
      ...
      switch (value.filterType) {
        case 'number': {
          if (operator) {
            filter = {
              [operator]: [
                { [fieldKey]: this.getNumberFilter(fieldKey, value.condition1, txnId) },
                { [fieldKey]: this.getNumberFilter(fieldKey, value.condition2, txnId) },
              ],
            };
          } else {
            filter = { [fieldKey]: this.getNumberFilter(fieldKey, value, txnId) };
          }
          break;
        }
        case 'text': {
          if (operator) {
            filter = {
              [operator]: [
                { [fieldKey]: this.getTextFilter(fieldKey, value.condition1, txnId) },
                { [fieldKey]: this.getTextFilter(fieldKey, value.condition2, txnId) },
              ],
            };
          } else {
        ...
        case 'date': {
          if (operator) {
            filter = {
              [operator]: [
                { [fieldKey]: this.getDateFilter(fieldKey, value.condition1, txnId) },
                { [fieldKey]: this.getDateFilter(fieldKey, value.condition2, txnId) },
              ],
            };
          } else {
        ...
        default: {
          console.error(`There was an error filtering grid data: ${value.filterType}`, {
            body: {
              txnId,
  ...
  private getNumberFilter(key, item, txnId): any {
    switch (item.type) {
      ...
      default:
        console.error(`There was an error filtering the numbers: ${item.type}`, {
          body: {
            txnId,
            message: 'Unknown filter type',
            timestamp: Date.now()
  ...
  private getTextFilter(key, item, txnId): any {
    switch (item.type) {
      ...
      case 'endsWith':
        return { $regex: `${MongoDBUtilities.escapeRegex(item.filter)}$` };
      default:
        console.error(`There was an error filtering the text range filter: ${item.type}`, {
          body: {
            txnId,
            message: 'Unknown filter type',
            timestamp: Date.now()
  ...
  private getDateFilter(key, item, txnId): any {
    ...
    switch (item.type) {
      case 'equals':
        return { $lte: new Date(moment.utc(date).endOf('day').toISOString()), $gte: new Date(moment.utc(date).startOf('day').toISOString()) };
      case 'notEqual':
        return { $not: { $lte: new Date(moment.utc(date).endOf('day').toISOString()), $gte: new Date(moment.utc(date).startOf('day').toISOString()) } };
      case 'greaterThan':
      ...
      case 'inRange': {
        ...
        return { $lte: new Date(moment.utc(utcDateTo).endOf('day').toISOString()), $gte: new Date(moment.utc(date).startOf('day').toISOString()) };
      }
      default:
        console.error(`There was an error filtering the date range: ${item.type}`, {
          body: {
            txnId,
            message: 'Unknown filter type',
            timestamp: Date.now()
  ...
  private getSetFilter(key, item): any {
    ...
    return {
      $in: values.map((value) =>
        (value === 'true' || value === 'false') ? ((value === 'true') ? true : false) : ((key === 'calculatedValue' || key === 'closedDealCalculatedValue' || key === 'closedDeals.calculatedValue') && value ? parseInt(value) : value)
      )
    };
  }
